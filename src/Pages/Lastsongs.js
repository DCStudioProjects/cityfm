    ...
    useEffect(() => {
        const Fetch = async () => {
            ...
            const promises = lastsongs.map(async (song) => {
                ...
                var photo = json?.album?.image[4]["#text"] !== undefined ? json?.album?.image[4]["#text"] : 'https://i.ibb.co/dpnXYhh/CityFM.jpg';
                var photo = json?.album?.image[4]["#text"] !== '' ? json?.album?.image[4]["#text"] : 'https://i.ibb.co/dpnXYhh/CityFM.jpg'; console.log(photo)
                const songData = {
                    id1: song.id,
                    id2: song.id + 1,
